...
export default function ParthnerCalendar() {
  ...
  return (
				...
				<div className={styles.left}>
					{loading ? (
						...
						<div className={styles.leftCards}>
							<div style={{paddingLeft: '10px', marginBottom:'20px'}}>Выберите ваш кемпинг:</div>

							{campings && campings.map(camp => (
								<div key={camp.id} onClick={() => handleCampClick(camp)} className={styles.campCards}>
        ...
        <div className={styles.right}>
					...
					{selectedCamp ? (
						<>
							<div className={styles.campName} >{selectedCamp.title}</div>
							{/* <p>ID отеля: {selectedCamp.id}</p> */}
							<h3 className={styles.bronya}>Бронирования:</h3>

              <div className={styles.cards}>
                {bookings === null ? (<p style={{margin: '10px'}}>Нет бронирований</p>)
                  :
